...
public final class Tesla extends Car{
    ...
    @Override
    public void start() {
        System.out.println("To start "+getMake()+" "+getModel()+" say \"Start\"");;
    }

    //overridden setModel() method
    public void setModel(String model) {
        ArrayList<String> models = new ArrayList<>();
        models.addAll(Arrays.asList("Model S", "Model Y", "Model x", "Model 3"));

        if (! models.contains(model) ){ //if specified mode it not valid model
            System.err.println("Invalid Tesla Model: "+model);
            System.exit(1);
        }
        super.setModel(model); //this.mode = model;
    }
