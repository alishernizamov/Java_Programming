...
public class PolymorphismPractice3 {
    ...
    //Print the name and Job Title of every employee
        //    ref type       ref object
        for (Employee each : employees){ //Employee: parent, employee: child
            System.out.println(each.getName()+" : "+each.getJobTitle() );
        }
        System.out.println("-----------------------2ND to 5TH TASKS-------------------------------------");
        //2. How many developers, testers, drivers, teachers do we have?
        ...
        for (Employee each : employees){
            ...
            if (each instanceof Tester){ //if the employee is Tester
                countTester++;
            }
            if (each instanceof Driver){ //if the employee is Driver
                countDriver++;
            }
            if (each instanceof Teacher){ //if the employee is Teacher
                countTeacher++;
            }
